...
- **Rate-limit aware** - Discord 429 responses are retried using the delay Discord returns, and messages to the same webhook are sent in order. Ordering is preserved per webhook, so a rate-limited message delays the ones queued behind it: a burst arrives late rather than out of sequence
