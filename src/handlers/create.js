...
function handleCreate(payload, event) {
  const { ref_type, repository } = payload;

  if (ref_type === "tag") {
    const key = `tag:${repository.full_name}:refs/tags/${payload.ref}`;
    bufferEvent(key, payload, (finalPayload) => sendCreateMessage(finalPayload, event), 5000);
  } else {
    sendCreateMessage(payload, event);
  }
}

function sendCreateMessage(payload, event) {
